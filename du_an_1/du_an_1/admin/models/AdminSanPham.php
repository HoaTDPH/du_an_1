...
public function insertSanPham($ten_san_pham,$gia_san_pham,$gia_khuyen_mai,$so_luong,$ngay_nhap,$thuong_hieu_id,$trang_thai, $mo_ta,$hinh_anh)
    {
        try {
            $sql = "INSERT INTO san_phams(ten_san_pham,gia_san_pham,gia_khuyen_mai,so_luong,ngay_nhap,thuong_hieu_id,trang_thai, mo_ta,hinh_anh)
             VALUES (:ten_san_pham,:gia_san_pham,:gia_khuyen_mai,:so_luong,:ngay_nhap,:thuong_hieu_id,:trang_thai, :mo_ta,:hinh_anh)";
            $stmt = $this->conn->prepare($sql);
            $stmt->execute(
                [
    ...
    public function updateSanPham($id, $ten_san_pham,$new_file ,$gia_san_pham,$so_luong,$ten_thuong_hieu,$trang_thai)
    {
        try {

            $sql = "UPDATE san_phams SET ten_san_pham = :ten_san_pham,hinh_anh = :hinh_anh, gia_san_pham = :gia_san_pham, so_luong = :so_luong,trang_thai = :trang_thai WHERE id = :id";
            $stmt = $this->conn->prepare($sql);
            
            // Fixed
            if(!isset($new_file)){
                $new_file = $this->getDetailSanPham($id)['hinh_anh'];
            }
